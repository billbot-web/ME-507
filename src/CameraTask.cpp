 ...
 */
uint8_t CameraTask::exec_wait() noexcept
{
    if (!instance_) return WAIT;

    uint8_t mode = 0;
    if (instance_->camMode_) {
        mode = instance_->camMode_->get();
    }
 ...
 */
uint8_t CameraTask::exec_capture_send_err() noexcept
{
    ...
    if (!instance_->camera_->captureAndFindError(fb, xerr, yerr, found))
        return CAP_SEND_ERR;

    // Update shares
    if (instance_->x_err_)  instance_->x_err_->put(xerr);
    if (instance_->y_err_)  instance_->y_err_->put(yerr);
 ...
 */
uint8_t CameraTask::exec_capture_send_return() noexcept
{
    if (!instance_->camera_) return WAIT;
    ...
    instance_->camera_->sendFrameWithError(Serial, fb, found, xerr, yerr);
    instance_->camera_->returnFrame(fb);
